...
#if KYBER_K == 2
  poly_getnoise_eta1_4x(skpv.vec+0, skpv.vec+1, tmp.vec+0, tmp.vec+1, noiseseed, 0, 1, 2, 3);
...
//We needn't generate e.
  ...
  poly_getnoise_eta1_4x(skpv.vec+0, skpv.vec+1, skpv.vec+2, tmp.vec+0, noiseseed, 0, 1, 2, 3);
  ...
  memcpy(kyber_sk1+KYBER_INDCPA_SECRETKEYBYTES, kyber_pk, KYBER_INDCPA_PUBLICKEYBYTES);
...
int main(void)
{
  ...
  for(i=0;i<NTESTS;i++) {
    ...
    // printf("\n");
    // fprintBstr(fp_rsp, "kyber_pk1 = ", kyber_pk, CRYPTO_PUBLICKEYBYTES);
